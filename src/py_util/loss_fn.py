...
class TOADLoss(torch.nn.Module):
    def __init__(self, trans_dim, trans_param, num_no_adv=None, tot_epochs=20, rho_adv=False, gamma=10,
        ...
        super(TOADLoss, self).__init__()

        self.rec_loss = ReconstructionLoss()
        self.trans_loss = TransformationLoss(
            dim=trans_dim,
            l=trans_param,
            use_cuda=use_cuda
        )
    ...
    def forward(self, pred_info, labels, compute_adv_loss=True, print_=False):
        text_l = pred_info["text_l"].type(torch.IntTensor)
        topic_l = pred_info["topic_l"].type(torch.IntTensor)
        if self.use_cuda:
